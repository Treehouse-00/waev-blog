...
This is a playbook for using <span class="waev-brand">Waev</span> to move from "I think there's a gap" to "here's the evidence, and here's where to place the repeater."

<div class="callout">
  <p>In short: Waev gives you three independent readings of the same coverage problem — contours on the Live Map, signal trends in Network Stats, and path evidence in the packet log. Read them together before you commit to a site, and you'll have a placement decision backed by data, not a drive on a hunch.</p>
</div>
...
**SNR at the edge of acceptable.** A working MeshCore link generally needs at least +5 to +7 dB SNR in typical conditions. Under that threshold, you're in weather-dependent territory. A node holding at +1 or +2 dB is technically alive but not reliably useful. Pull the time series in Stats to see whether the degradation is constant or shows patterns — weather, terrain obstructions, interference at certain hours.
...
Not to confirm the gap exists — a node at the edge of coverage will show its degraded metrics in Network Stats regardless of where your observers are positioned. But to *map* the gap, you do need observer coverage in the area. The Live Map draws topology from what observers recorded. If all your observers sit on the backbone, the backbone will appear clear and edge nodes will appear intermittently or not at all. An observer placed in or near the suspect area dramatically improves map fidelity for that region — and makes the post-placement validation much easier to read.

### The map shows silence, but members in that area say the mesh is working. Who's right?

Both. Short messages at low traffic pass through marginal links that would fail under sustained load. Trust the members for operational reports; use the Stats data for infrastructure decisions. A link at +1 dB SNR with 2 packets per hour is working *today* — but it's not the foundation you want when it counts. The data tells you the margin. The margin tells you the risk.

### Can I do a manual field survey using Live Packets?

Yes. If you're carrying a MeshCore device that connects to your MQTT observer network — and the node doesn't carry a privacy opt-out marker — transmissions will appear in Live Packets. Drive or walk the edge of the suspect zone and watch whether packets are reaching the network, and over how many hops. It's a manual survey and it works. Pair it with the Stats view afterward to correlate what you saw in the field with what the data shows over time.

### What does the before/after look like in Network Stats?

The clearest signal is SNR improvement on the links that were degraded, followed by hop-count reduction for the nodes that were routing around the gap. In the time series, the repeater's commissioning time appears as an inflection — both metrics step up together. If neither moves after 48 hours, the candidate site may not be close enough to the actual gap, or the anchor node's own path quality needs attention first.

---

Coverage problems aren't mysterious. They're geometry, terrain, and signal math — and they all leave traces in packet data. <span class="waev-brand">Waev</span> gives you a window into those traces. The judgment call — where exactly to reach, what to power the repeater from, how to get it up the hill — is still yours. That call gets considerably easier when you have evidence underneath it.
