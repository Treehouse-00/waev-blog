...
description: "Duplicate packet floods in a LoRa mesh waste airtime and hide real problems. Here is what causes them, how MeshCore suppresses them, and how to spot the difference in Live Packets."
date: 2026-11-17
...
MeshCore's answer: the first message to a new destination floods through every repeater until it arrives. Each repeater appends its identifier to the path field. The destination reverses the path and sends it back. From that point, all direct messages between those two contacts travel along the cached route — and only the repeaters on that route handle the packet. This is the flood-once-then-direct model described in [Flood First, Then Direct](/blog/flood-first-then-direct/).
