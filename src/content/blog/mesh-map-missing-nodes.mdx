...
If you added the marker intentionally (a household member's personal device, a node you've chosen to exclude), this is correct behavior. If you added it accidentally while setting up a node name, removing the marker and rebooting the node will cause it to start appearing. For more on how opt-out works and what gets dropped vs. scrubbed, see [what Waev chooses not to know](/blog/what-we-choose-not-to-know/).

**4. Observer not publishing to Waev's ingest.** The observer is on the mesh, hearing RF traffic, but its MQTT connection to Waev's ingest is down. It could be an internet outage, a credential issue, or a TLS certificate problem. The observer hears the node; it just can't forward the observation.
