...
description: "A practical LoRa dev board comparison for MeshCore — Heltec V3, Heltec V4, and RAK WisBlock 4631 evaluated by TX power, sleep current, and the use case each handles best."
date: 2026-09-22
...
faq:
  - q: "Which LoRa dev board should I start with for MeshCore?"
    a: "The Heltec V3 is the most common starting point — it's inexpensive (roughly $18–25 depending on source), has a built-in OLED display, onboard LiPo charging, WiFi and BLE, and first-class MeshCore firmware support. If you're buying fresh today and want more transmit power, the V4 is the natural upgrade at a similar price."
  ...
  - q: "When should I choose RAK WisBlock over Heltec for MeshCore?"
    a: "When power budget matters more than anything else. The RAK 4631 module uses an nRF52840 which sleeps below 10 microamps — substantially lower than the ESP32-S3 in the Heltec boards. For a solar-powered repeater that needs to run months unattended on a small panel, the nRF52840 sleep current is a meaningful advantage. The trade-off is no WiFi and more assembly work."
  - q: "Do I need WiFi on a MeshCore node?"
    a: "Only for MQTT gateway and web configuration use cases. A Companion node just needs Bluetooth. A Repeater configured entirely via serial or the web config tool at config.meshcore.io does not need WiFi at runtime. If your node role is a remote solar repeater that you configure once and leave, WiFi is unnecessary."
...
If you're starting from scratch and want a board that won't fight you, start with the V3 (or its V4 successor if buying new). The community support is the largest of any platform in this space.
...
What the 4631 does better than any ESP32 board: sleep current. The nRF52840 sleeps below 10 µA in power-saving mode. Compare that to 15 µA for the Heltec on VBAT — a difference that compounds over the many months a remote repeater runs unattended. For a solar-powered node that spends most of its time dormant, the nRF52840's power profile is not just incrementally better; it's structurally different.
