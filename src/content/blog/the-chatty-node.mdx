...
description: "How to spot and fix a chatty node on a LoRa mesh — diagnosing too-frequent adverts in Live Packets, identifying the source node, and the CLI command that fixes it."
date: 2026-09-29
...
faq:
  - q: "What is a chatty node on a LoRa mesh?"
    a: "A chatty node is a MeshCore repeater or sensor that sends flood advertisement packets far more frequently than the intended interval — often every few minutes instead of the default 47 hours. Every advert floods through all relay neighbors, consuming shared channel time and pushing useful traffic out."
...
Since firmware v1.16.0 (released April 2026), the default `flood.advert.interval` on a MeshCore repeater is 47 hours. Older firmware defaulted to 12 hours. Either way: adverts are measured in hours, not minutes.
