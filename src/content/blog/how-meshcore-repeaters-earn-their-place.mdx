...
faq:
  ...
  - q: "What is the difference between an enrolled observer and an authenticated repeater in Waev?"
    a: "An enrolled observer is a MeshCore node publishing to Waev's MQTT ingest over the internet — it hears RF packets and publishes them as telemetry to Waev. An authenticated repeater is a node running repeater firmware whose public-key identity is established on the mesh. Waev draws topology edges only from what enrolled observers witness about authenticated repeaters, combining both sources to build the map."
---
...
Waev's topology is built from what enrolled observers report over MQTT. An enrolled observer is a MeshCore node — typically running on a fixed installation — that has both a radio interface and an internet connection. It hears RF packets on the mesh and publishes them directly to Waev's own MQTT ingest, using a credential that can only send that observer's own data. The observer is the bridge between the radio world and the analytics layer.

<Figure caption="<b>The observer bridges radio and network.</b> On the left, it hears RF packets from mesh nodes. On the right, it publishes over MQTT/TLS to Waev's ingest with a publish-only credential. Waev has no antenna — it sees only what enrolled observers relay.">
  <HiwObserverBridge />
</Figure>
...
Each edge represents: a packet traveled this path; an enrolled observer publishing its stream directly to Waev's ingest witnessed it; the repeater in that path signed its advertisement with a private key it generated at first boot and has never transmitted. That is the chain.
...
For more on how Waev's never-captive stream keeps that MQTT connection under operator control, see [The Never-Captive Stream](/blog/bring-your-own-broker/).
