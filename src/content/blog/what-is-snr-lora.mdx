...
description: "SNR — Signal-to-Noise Ratio — is the number Waev shows for every LoRa observation. This primer explains what it measures, what the dB values mean, and why LoRa can decode below zero dB SNR."
date: 2026-10-13
...
The +7 dB figure as a working target appears in Waev's Network Stats watchlist logic: paths consistently below +7 dB get flagged as marginal. It's the same figure used in the [Reading the Signal](/blog/reading-the-signal/) post, which covers how to use SNR alongside hop count to assess overall path quality.
...
[When the grid goes down](/blog/when-the-grid-goes-down/) covers the practical work of using SNR trends to build a pre-event network health baseline.
