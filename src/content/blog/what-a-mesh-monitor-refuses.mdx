...
faq:
  ...
  - q: "Does the never-captive stream change what Waev collects?"
    a: "No. The data model is the same regardless of where the stream goes. An enrolled observer publishes directly to Waev's own MQTT ingest with a publish-only credential — it can send its data and nothing else — and the operator can read that data back out or point the same stream at other tools. Waev's collection policy is applied at the ingest edge, before anything is written, independent of where else the stream is sent."
---
...
<div class="callout">
<p><strong>TLDR:</strong> Privacy-first mesh monitoring means the tool refuses to collect what it doesn't need before any data is stored — not after. Identity fields, location inference, and device tracking are declined at the ingest edge, structurally, so they cannot be retrieved, breached, or accidentally re-enabled. What doesn't enter the database can't be accessed, misused, or lost.</p>
</div>
...
Waev is a tool for network operators who need to know whether their infrastructure is working. [The evidence-based topology model](/blog/how-waev-reads-your-mesh/) means the map shows only what can be verified from enrolled observers. [The never-captive stream](/blog/bring-your-own-broker/) means the operator's data reaches Waev over a publish-only credential and can be read back out, or pointed at other tools, without anything about the stream being captive to Waev. The refusal to collect identity, location, and personal device tracking is the third leg of the same stance: the network intelligence you need, without the surveillance infrastructure you don't.
