...
description: "An evidence based mesh network map draws only edges it can verify — enrolled observers and authenticated repeaters. No proof, no line. That restraint is the feature."
date: 2026-09-13
...
faq:
  ...
  - q: "Why would a mesh map show connections that don't actually exist?"
    a: "Most LoRa mesh nodes broadcast ADVERT packets that include information about their neighbors. A naive mapping tool can draw edges from those neighbor lists without verifying that the claimed link is current, functional, or even real. Nodes can also carry stale neighbor tables or be deliberately misconfigured. An evidence-based approach ignores claimed neighbor relationships and draws only from direct RF observations made by enrolled observers."
