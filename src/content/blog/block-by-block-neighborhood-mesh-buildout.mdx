...
faq:
  ...
  - q: "Who owns the data from a neighborhood mesh network?"
    a: "An enrolled observer publishes its packet stream to Waev's own MQTT ingest using a credential that is publish-only — it can send its observer's data and nothing else, and it cannot read, alter, or delete anything on the broker. The community reads its data back out through a public read API, and the same observer stream can be pointed at other tools of the community's choosing using the presets Waev ships in its own docs. Nothing about the stream is captive to Waev."
---
...
The repeater is also the right infrastructure to put [an observer](/blog/how-waev-reads-your-mesh/) on. A rooftop repeater already has the height advantage and the RF view of the neighborhood. An observer co-located with the repeater gives you the best possible vantage for watching what the mesh is doing, publishing its stream directly to Waev's ingest.
