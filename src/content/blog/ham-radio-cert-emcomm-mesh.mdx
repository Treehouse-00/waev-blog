...
faq:
  ...
  - q: "What is the minimum kit for a CERT mesh go-kit?"
    a: "A minimal CERT mesh go-kit for a single deployment point includes: one or more MeshCore nodes for field operations; one observer node (a MeshCore device with MQTT gateway firmware, or equivalent) with internet connectivity at the operations center, enrolled to publish its packet stream to Waev's own MQTT ingest; and a Waev account reading that stream back out. The observer is the critical component — without it, the mesh operates but is not visible on the Live Map."
...
**Node status and position.** A MeshCore node that transmits periodically sends signal quality and hop-count data that an enrolled observer publishes directly to Waev's ingest. From there, Waev shows which nodes are active, whether any have gone silent, and whether hop counts have changed — meaning paths have changed. During an activation, this is a live picture of your data infrastructure's health.
...
**One observer node.** This is a MeshCore-compatible device running gateway firmware that connects to the internet over TCP/IP. Positioned at your operations center, where internet access is available. This is the bridge between the radio mesh and the Live Map. Without it, the mesh operates but is not visible.

**An enrolled observer credential.** The observer publishes its packet stream directly to Waev's own MQTT ingest using a credential that is publish-only — it can send its own data and nothing else; it cannot read, alter, or delete anything on the broker. There's no separate broker for you to stand up or maintain. You read your data back out through a public read API, and the same observer stream can be pointed at other tools of your choosing using presets Waev ships in its own docs — nothing about the stream is captive to Waev. A served agency asking about data custody should be pointed at Waev's own terms and privacy policy, not the operator's infrastructure — the custody question has moved: your packet stream now reaches Waev's ingest directly instead of sitting on equipment the agency or operator controls.

**A Waev account reading that stream.** This is the analytics layer. Enroll the observer before any exercise and start the baseline. By activation day, you'll have weeks of context.
