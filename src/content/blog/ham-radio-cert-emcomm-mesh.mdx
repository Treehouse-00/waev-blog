...
**An enrolled observer credential.** The observer publishes its packet stream directly to Waev's own MQTT ingest using a credential that is publish-only — it can send its own data and nothing else; it cannot read, alter, or delete anything on the broker. There's no separate broker for you to stand up or maintain. You read your data back out through a public read API, and the same observer stream can be pointed at other tools of your choosing using presets Waev ships in its own docs — nothing about the stream is captive to Waev. A served agency asking about data custody gets an answer grounded in that model, not operator-hosted custody.
{/* CANON-GAP: exact data-custody and retention specifics for a served-agency question (what's retained, for how long, and under what request process) are not yet confirmed */}
