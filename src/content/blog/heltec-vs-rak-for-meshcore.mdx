...
description: "Heltec vs RAK for MeshCore: the WiFi LoRa 32 and the WisMesh Pocket V2 are different tools for different jobs. Here is how to pick the right one for your build."
date: 2026-06-09
...
The WiFi radio is the differentiating factor. An ESP32-based node is the most common choice for an MQTT observer: it connects to your home network, runs MeshCore firmware, and publishes observations directly to Waev's own ingest without any additional hardware. It's the standard gateway board.
...
There's no WiFi. The WisMesh Pocket V2 has Bluetooth for phone pairing, but it cannot connect to Waev's MQTT ingest directly. If you want to use it as a Waev observer — publishing packets to Waev's ingest for the Live Map — you need either a separate gateway node on the same mesh, or to route it through an existing WiFi-connected observer.
